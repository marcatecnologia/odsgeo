...
class GeoServerParcelasService
{
    ...
    public function __construct()
    {
        ...
        $this->municipiosLayer = 'br_municipios_2024';
        ...
        Log::info('GeoServerParcelasService inicializado', [
            ...
            'municipiosLayer' => $this->municipiosLayer,
     ...
     */
    public function getParcelasPorMunicipio(string $codigoMunicipio, int $page = 1, int $perPage = 50): array
    {
        try {
            $cacheKey = "parcelas_municipio_{$codigoMunicipio}_page_{$page}";
            
            if ($this->cacheEnabled && Cache::has($cacheKey)) {
                Log::info('Retornando dados do cache', ['cacheKey' => $cacheKey]);
            ...
            if ($this->cacheEnabled) {
                ...
                Log::info('Dados salvos no cache', ['cacheKey' => $cacheKey]);
            }
     ...
     */
    public function getMunicipiosPorUF(string $uf): array
    {
        try {
            ...
            $params = [
                ...
                'srsName' => 'EPSG:4674',
                'CQL_FILTER' => "uf='{$uf}'",
                'propertyName' => 'codigo_municipio,nome_municipio,uf,geom'
            ];
            ...
            if (!$response->successful()) {
                ...
                return [
                    'success' => false,
                    'error' => 'Erro ao buscar municípios no GeoServer'
                ];
            }

            $data = $response->json();
            
            $municipios = array_map(function($feature) {
                return [
                    'codigo' => $feature['properties']['codigo_municipio'],
                    'nome' => $feature['properties']['nome_municipio'],
