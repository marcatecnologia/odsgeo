...
class PlanilhaOdsService
{
    public function exportar(PlanilhaOds $planilha)
    {
        ...
        $sheet1->setCellValue('B6', $planilha->area_imovel);
        ...
        $sheet3->setCellValue('B6', $planilha->proprietario_percentual . '%');
        ...
        foreach ($planilha->vertices()->orderBy('ordem')->get() as $vertice) {
            ...
            $sheet4->setCellValue('B' . $row, $vertice->coordenada_x);
            $sheet4->setCellValue('C' . $row, $vertice->coordenada_y);
            $sheet4->setCellValue('D' . $row, $vertice->altitude);
        ...
        foreach ($planilha->segmentos()->orderBy('ordem')->get() as $segmento) {
            ...
            $sheet5->setCellValue('C' . $row, $segmento->azimute);
            $sheet5->setCellValue('D' . $row, $segmento->distancia);
